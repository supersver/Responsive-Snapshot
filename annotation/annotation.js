...
document.getElementById('lineWidth').addEventListener('change', e => { lineWidth = parseInt(e.target.value, 10); });

/* Undo / Clear */
document.getElementById('undoBtn').addEventListener('click', () => { annotations.pop(); redraw(); });
document.getElementById('clearBtn').addEventListener('click', () => { annotations = []; redraw(); });
...
overlayCanvas.addEventListener('mousedown', e => {
  ...
  if (currentTool === 'pen') {
    ...
    if (text) {
      annotations.push({ type: 'text', x: pos.x, y: pos.y, text, color: strokeColor, size: lineWidth * 5 + 10 });
      redraw();
    }
  }
});
...
overlayCanvas.addEventListener('mousemove', e => {
  ...
  if (currentTool === 'pen') {
    ...
    clearOverlay(); drawPenPath(overlayCtx, currentPath, strokeColor, lineWidth);
  } else if (currentTool === 'arrow') {
    clearOverlay(); drawArrow(overlayCtx, startX, startY, pos.x, pos.y, strokeColor, lineWidth);
  } else if (currentTool === 'rect') {
    clearOverlay(); drawRect(overlayCtx, startX, startY, pos.x, pos.y, strokeColor, lineWidth);
  } else if (currentTool === 'blur') {
    clearOverlay(); drawBlurPreview(overlayCtx, startX, startY, pos.x, pos.y);
  }
});
...
overlayCanvas.addEventListener('mouseup', e => {
  ...
  const pos = getPos(e);
  if (currentTool === 'pen') {
    annotations.push({ type: 'pen', path: [...currentPath], color: strokeColor, width: lineWidth });
  } else if (currentTool === 'arrow') {
    annotations.push({ type: 'arrow', x1: startX, y1: startY, x2: pos.x, y2: pos.y, color: strokeColor, width: lineWidth });
  } else if (currentTool === 'rect') {
    annotations.push({ type: 'rect', x1: startX, y1: startY, x2: pos.x, y2: pos.y, color: strokeColor, width: lineWidth });
  } else if (currentTool === 'blur') {
    annotations.push({ type: 'blur', x1: startX, y1: startY, x2: pos.x, y2: pos.y });
  }
  clearOverlay(); redraw();
});

overlayCanvas.addEventListener('mouseleave', () => { if (isDrawing) { isDrawing = false; clearOverlay(); } });
...
function redraw() {
  ...
  annotations.forEach(a => {
    switch (a.type) {
      case 'pen':   drawPenPath(mainCtx, a.path, a.color, a.width); break;
      case 'arrow': drawArrow(mainCtx, a.x1, a.y1, a.x2, a.y2, a.color, a.width); break;
      case 'rect':  drawRect(mainCtx, a.x1, a.y1, a.x2, a.y2, a.color, a.width); break;
      case 'text':  drawText(mainCtx, a); break;
      case 'blur':  drawBlur(mainCtx, a.x1, a.y1, a.x2, a.y2); break;
    }
  });
}
...
function drawText(ctx, a) {
  ctx.font = 'bold ' + a.size + 'px sans-serif'; ctx.fillStyle = a.color; ctx.fillText(a.text, a.x, a.y);
}
...
function drawBlurPreview(ctx, x1, y1, x2, y2) {
  ctx.strokeStyle = 'rgba(255,255,255,0.5)'; ctx.lineWidth = 1; ctx.setLineDash([5,5]);
